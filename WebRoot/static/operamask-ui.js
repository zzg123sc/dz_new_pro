...
$.omWidget("om.omDraggable", $.om.omMouse, {
	...
	_createHelper: function(event) {
		...
		if(!helper.parents('body').length)
			var temp=RegExp('^td|^TD').test(helper[0].tagName)?$(this.element[0].parentNode).closest('table').parent():this.element[0].parentNode;
			helper.appendTo(temp);
			...
			if($body.data("loading")){
		 	...
		 	$.ajax(url , {
		 		...
		 		success: function(data, textStatus, jqXHR){
		 			...
		 			typeof(dialogLoadFn)=='function'&&dialogLoadFn(self);
		 		},
    ...
    $.omWidget('om.omButton', {
        ...
        _init : function(){
            ...
            $newelement.bind( 'mouseenter',function( event ){
            				...
            				}).unbind( "click.omButton").bind( "click.omButton", function( event ){
            					if ( options.disabled ) {
            						event.preventDefault();
            						event.stopImmediatePropagation();
            						return false;
            					...
            					//导出按钮添加禁用事件
				   			    if (options.title=="查询" || options.title=="导出") {
        ...
        _initButton : function(){
            ...
            !options.icons.left && !options.icons.right && element.closest('.om-btn').addClass('omBtnIsText');
        },
        _getLabel : function(){
	...
	$.omWidget('om.omCombo', {
        ...
        _init:function(){
            ...
            if (options.width != 'auto') {
                ...
                function inputElWfn(){inputEl.width(span.innerWidth() - inputEl.next().outerWidth()-10);};
                inputEl.next().outerWidth()>=0.1?inputElWfn():setTimeout(inputElWfn,100);//修复omCombo方法下拉框的箭头移位问题。
           //   inputEl.width(span.innerWidth() - inputEl.next().outerWidth()-10);
            }
            ...
            if (this.element.closest('.edus_search').length){
            ...
            if (unusable) {
                ...
                this.textInput.addClass('grayInput');
            } else {
            	if(!options.hasInitLoad) {
                	this._bindEvent();
            	}
            }
        },
        _create:function(){
            ...
            this.textInput = valueEl.clone().removeAttr("id").removeAttr("name").removeAttr("needfilter").removeAttr("prevname").appendTo(span);
            ...
            var selectOne = valueEl.next();
            if(selectOne.hasClass('edus_selectOne')){selectOne.attr('id','selectOneB').css('padding',0);};
        },
        /**
        ...
        changeComboShowData:function () {
        	    var self = this, options = self.options,input = self.textInput,valueEl = self.element;
        		//点击时过滤下拉内容（针对自定义过滤显示）
            	var tmpNeedShow = valueEl.attr("needShowData");//需要显示
            	var tmpNoShow = valueEl.attr("noShowData");//不需要显示
            	//判断是否有改变
            	var dqVal = self.value();
            	var tmpVal = "";
            	var nowData = options.oldData!=null?options.oldData:self.getData();
            	if (nowData==null || nowData.length==0 || (options.tableId!=null && options.tableId>0 && !options.ifAjaxData)) {
            		return nowData;
            	}
            	if (tmpNeedShow!=null && tmpNeedShow!=undefined && tmpNeedShow.length>0) {
            		options.oldData = nowData;//存放一个原数据
            		tmpNeedShow = ","+tmpNeedShow+",";
            		var newData = [];
            		$(nowData).each(function (i,one) {
            			if (tmpNeedShow.indexOf(","+one.value+",")>=0) {
            				newData[newData.length]=one;
            				//判断当前字段是否存在
            				if (dqVal!=null && dqVal!="" && one.value==dqVal) {
            					tmpVal = dqVal;
            				}
            			}
            		});
            		self._loadData(newData);
            	} else if (tmpNoShow!=null && tmpNoShow!=undefined && tmpNoShow.length>0) {
            		options.oldData = nowData;//存放一个原数据
            		tmpNoShow = ","+tmpNoShow+",";
            		var newData = [];
            		$(nowData).each(function (i,one) {
            			if (tmpNoShow.indexOf(","+one.value+",")<0) {
            				newData[newData.length]=one;
            				//判断当前字段是否存在
            				if (dqVal!=null && dqVal!="" && one.value==dqVal) {
            					tmpVal = dqVal;
        ...
        _bindEvent:function(){
            ...
            input.unbind("focus").focus(function(){
                //修复搜索条件内打开的下拉列表不能关闭问题。
            	//function globalEventClear(e){!$(e.target).parent().hasClass('om-droplist')&&(dropList.hide(),$(document).unbind('mousedown',globalEventClear));};
                //$(document).unbind('mousedown',globalEventClear).bind('mousedown',globalEventClear);
            	self.changeComboShowData(data);//隐藏显示值的处理
                if(isFocus) 
                    return;
                isFocus = true;
                ...
                if (!self.dataHasLoaded) {
                    if(!expandTrigger.hasClass('om-loading')){
                        ...
                        if (typeof(options.dataSource) == 'object') {
                        	if(options.eachAjaxLoad) {
                        		var data = {};
            					var toHtData = valueEl.lmfCombo("getFilterData",data);
            					if(data.noInfoPrompt != null && data.noInfoPrompt.length > 0) {
            						self.setData([]);
            						$.omMessageTip.show({
										title : '提示',
										content : data.noInfoPrompt,
										timeout : 1000
									});
            						return;
            					}
                        		...
                        		if(tmpJsonData != options.prevTmpJsonData) {
			    					options.prevTmpJsonData = tmpJsonData;
                        			//获取当前弹层选中的值
            						var inputData = valueEl.data("inputData");
            						if (inputData!=null && inputData.length>0) {
            							var checkIds = "";
            							for (var i=0;i<inputData.length;i++) {
            								eval("checkIds+=(i>0?',':'')+inputData[i].T_"+options.tableId+"_0");
            							}
            							tmpJsonData+="&orderDataId="+checkIds;
            						}
            ...
            }).unbind('mousedown.myBlur').bind('mousedown.myBlur', function(e){
				if (this.getAttribute('readonly') || this.getAttribute('disabled') || this.myBlur) return;
				var self = this, mySelf = mySelf || $(this);
				self.myType = self.myType || self.type;
				self.type = 'button';
				this.myBlur = 1;
				setTimeout(function(){self.focus();self.type = self.myType;self.select();},0);
			}).blur(function(e){
                this.myBlur = 0;
                isFocus = false;
            ...
            if(needFilter != null && needFilter.length > 0) {
            	...
            	var noInfoPrompt = "";
           		$(needFilterArr).each(function(i, one){
           			...
           			var tmpArr = $("input[name$='_" + tmpFieldId + "']");
           			//首先判断同级的 tr下有没有 然后判断父级的table下有没有
           			//我把这里之前的所有的用prevName做条件的都改为了name,因为现在得判断缺少一本分判断，而且name的效果也是一样的（王启玉）
           			var prevTrEle = el.closest("tr").find("input[name$='_" + tmpFieldId + "']");
           			var ifJx = true;
           			if(prevTrEle.length == 0) {
           				prevTrEle = el.closest("table").parent().closest("table").find("input[name$='_" + tmpFieldId + "']");
           				...
           				if(prevTrEle.length >0 && (prevTrEle.attr("id") + "_childTable") == el.closest("table").attr("id")) {
           					...
           					if(prevTrEle.length == 0) {
           						prevTrEle = el.closest("table").find("input[name$='_" + tmpFieldId + "']");
           					}
           				}
           			} 
           			if(ifJx) {
	           			...
	           			if(prevTrEle.length > 0) {
	           				if(prevTrEle.attr("type") == "hidden") {
	           					...
	           					if(tmpId == null || tmpId.length == 0) {
	           						var prevLabel = prevTrEle.closest(".edus_searchBox").prev("label");
	           						if(prevLabel.length > 0) {
	           							noInfoPrompt = "必须先填写：" + prevLabel.text() + " 否则不可以选择此项";
	           						}
	           					}
	           				} else if (prevTrEle.parent().hasClass('om-combo')) {//判断是否为数据字典字段
	           					tmpId = prevTrEle.omCombo('value');
	           					if(tmpId == null || tmpId == 0 || tmpId.length == 0) {
	           						var prevLabel = prevTrEle.closest(".om-combo").prev("label");
	           						if(prevLabel.length > 0) {
	           							noInfoPrompt = "必须先填写：" + prevLabel.text() + " 否则不可以选择此项";
	           						}
	           					}
           			...
           			data.noInfoPrompt = noInfoPrompt;
           		});
            }
            return data;
        },
        _dropListOpsition:function(valCon){// add by 占天祥
        	var dropListContainer=valCon.dropListContainer,
        	widgetContent = dropListContainer.find('>.om-widget-content'),
        	span=valCon.span,
        	options=valCon.options,
        	dropList=valCon.dropList;
            	self._dropListOpsition
            if (!options.listAutoWidth) {
                dropListContainer.width(span.outerWidth());
            }else{
            	if($.browser.msie&&($.browser.version == "7.0")&&!$.support.style){
            		dropListContainer.width(dropList.show().outerWidth()>span.outerWidth()?dropList.show().outerWidth()+17:span.outerWidth());
            	}else{
            		dropListContainer.width(dropList.outerWidth()>span.outerWidth()?dropList.outerWidth()+17:span.outerWidth());
            	}
            }
            ...
            //判断下拉菜单屏幕中出现位置进行定位
            widgetContent.css({'maxHeight': ''});
            var inputPos = span.offset(),
	            bodyheight = dropList.closest("body").height(),
	            inputTop = inputPos.top,
	            spanHeight = span.outerHeight(),
	            dropListHeight = dropListContainer.outerHeight(),
	            ifTop = (bodyheight - spanHeight)/2 > inputTop,
	            myDropListHei, myTop;
            if(ifTop){
	            myDropListHei = bodyheight - inputTop - spanHeight;
	            myTop = inputTop + spanHeight;
            }else{
            	myDropListHei = dropListHeight > inputTop-2 ? inputTop-2 : dropListHeight;
	            myTop = inputTop - myDropListHei;
	            dropList.css({'border-top':'1px solid #64a0fb'});
            };
            dropListContainer.css({
                'left': inputPos.left,
                'top': myTop,
            });
            widgetContent.css({'maxHeight': myDropListHei});
            var myInput = this.element.parent(), myDialog = myInput.closest('.om-dialog-content.om-widget-content'), setTopObj = myInput.TopObj = {spanTop : 0, myTop : myTop, inputTop : inputTop, setTop : 0};
            myDialog.unbind('scroll.myWheel').bind('scroll.myWheel',function(){
            	setTopObj.spanTop = span.offset();
            	setTopObj.setTop = setTopObj.inputTop - setTopObj.spanTop.top;
            	setTopObj.setTop && dropListContainer.css({
	                'left': setTopObj.spanTop.left,
	                'top': setTopObj.myTop - setTopObj.setTop,
	            });
            });
        },
        _showDropList:function(){
        	...
        	var valCon = {dropListContainer:dropListContainer,span:span,options:options,dropList:dropList};
            self._dropListOpsition(valCon);
            dropList.show();
        ...
        _ajaxLoad:function(url){
            ...
            $.ajax({
                ...
                success: function(data, textStatus){
                    ...
                    self._loadData(data);
                    self._toggleLoading('remove');
                    options.ifAjaxData = true;
                    options.oldData = data;
                    self.changeComboShowData();//隐藏显示值的处理
                },
        ...
        _backfill:function(source){
            ...
            if (enableMulti) {
            	...
            	var tmpChoose = $(source).hasClass('om-combo-selected');
            ...
            if (self.expandTrigger.siblings('.fieldShowHid').length)
            	dropList.find('.om-combo-list-row').length == dropList.find('.om-combo-selected').length && $(source).removeClass('om-combo-selected');
            if (dropList.css('display') == 'none') {
                return;
            }
            ...
            typeof(window.readyComboSize)=='function'&&window.readyComboSize(self.textInput,self);
        },
...
$.omWidget("om.omDialog", {
	...
	moveToTop: function(force, event) {
		...
		//修复弹层高度定位问题。
		$('.om-dialog.om-widget.om-widget-content.om-corner-all.om-draggable.om-resizable').each(function(){
			var ZmaxZ=parseInt($(this).css('z-index'));
			$.om.omDialog.maxZ=$.om.omDialog.maxZ>ZmaxZ?$.om.omDialog.maxZ:ZmaxZ;
		});
		if (self.overlay) {
			$.om.omDialog.maxZ += 1;
	...
	open: function() {
		...
		typeof(hideFormType11Btn) == 'function' && setTimeout(function(){hideFormType11Btn.apply(self, myArguments);},0);
	...
	_createButtons: function(buttons) {
		if(window.mobile){addDialogFn({self:this,options:this.options})};
		var self = this,
			hasButtons = false,
	...
	_setOption: function(key, value){
		...
		self.element.attr('boxsize', self.options.width+'@'+ self.options.height);
	},
...
(function($){
    ...
    $.omWidget('om.omFileUpload', {
        ...
        upload:function(index) {
            ...
            var fileFormat = element.attr('accept'), accept = '', inputFileOK = 0;
            if (fileFormat){
            	fileFormat = fileFormat.split(',');
            	for (var z=0,temp=[],len=fileFormat.length; z<len; z++) temp.push(fileFormat[z].split('/').pop());
            	fileFormat = temp.join(',');
            	accept = fileFormat.replace('jpeg', 'jpe?g').replace(/,/g,'|');
            };
            if (fileFormat) element.siblings('.om-fileupload-queue').find('.fileName').each(function(){
            	if (!RegExp(".(" + accept + ")$", "i").test(this.title)){
            		inputFileOK = 1;
            		if (this.children.length) return;
            		var myErrorMsgBlock = $('<span class="errorMsgBlock" style="top: 0;left: 100%;margin: 0 0 0 10px;">文件格式只能为：'+fileFormat+'</span>');
            		myErrorMsgBlock.click(function(){this.remove();});
            		$(this).css('position',' relative').append(myErrorMsgBlock);
            	};
           	});
            ...
            if(element&&element.dialoguploadFn)element.dialoguploadFn(this,element);
            if(typeof(index) != 'undefined'){
                if((fileId = _findIDByIndex(id,index)) === false) return;
            }
            if(document.getElementById(uploaderId).startFileUpload)
        ...
        cancel:function(index) {
            ...
            if(typeof(index) != 'undefined'){
				...
				if(parseInt(fileId.replace(/^drop/,''))){
					element[0][fileId+'abort']();
					$('#'+element.attr('id')+fileId).remove();
				}else
                document.getElementById(uploaderId).cancelFileUpload(fileId, true, true, false);
            } else{
                // cancel all
                if(document.getElementById(uploaderId).clearFileUploadQueue)
                document.getElementById(uploaderId).clearFileUploadQueue(false);else
                element.siblings('.om-fileupload-queue').children().remove();
            }
        },
        ...
        _setOption : function(key, value) {
            ...
            if(uploader.updateSettings&&$.inArray(key, dynOpts) != -1){
                uploader.updateSettings(key, value);
            }
        }, 
        ...
        _create : function() {
            ...
            if (settings.onInit() !== false) {
                ...
                //swfobject.embedSWF(settings.swf, settings.id + 'Uploader', settings.width, settings.height, '9.0.24', settings.expressInstall, data, {'quality':'high','wmode':settings.wmode,'allowScriptAccess':settings.scriptAccess},{},function(event) {if (typeof(settings.onSWFReady) == 'function' && event.success) settings.onSWFReady();});
                var myImg = $('<img class="omFileUploadImg" style="border-radius: 5px; visibility: visible; overflow: hidden; float: left; margin-right: 10px;">').click(function(){element.trigger('click');});
                element.after(myImg.attr('src', omFileUploadImg));
            ...
            element.bind("omFileUploadSelect", {'action': settings.onSelect, 'queueID': settings.queueID}, function(event, ID, fileObj) {
                ...
                fileuploadLen=element.siblings('#'+myId+'Queue').find('.om-fileupload-queueitem'),
				edus_tableTrTwoLen=element.siblings('#'+myId+'_file_upload_fileDiv').find('.edus_tableTrTwo'),
                fileNumber=element.fileNumber;
				if((fileuploadLen.length+edus_tableTrTwoLen.length)>=fileNumber){
                	if(!/^drop/.test(ID))setTimeout(function(){document.getElementById(element.attr('id') + 'Uploader').cancelFileUpload(ID, true, true, false);},300);
                	alert($.om.lang.omFileUpload.queueSizeLimitMsg+fileNumber+'.');
                	return;
                };
                if (self._trigger("onSelect",event,ID,fileObj) !== false) {
                    ...
                    $(queue).append('<div id="' + $(this).attr('id') + ID + '" class="om-fileupload-queueitem">\
                            ...
                            <span class="fileName" title="'+fileObj.name+'">' + fileName + ' (' + byteSize + suffix + ')</span><span class="percentage"></span>\
                            <div class="om-fileupload-progress">\
                                <div id="' + $(this).attr('id') + ID + 'ProgressBar" class="om-fileupload-progressbar"><!--Progress Bar--></div>\
                            </div>\
                        </div>');
                typeof(window.thInputClick)=='function'&&window.thInputClick(element);
                }
            });
            ...
            element.bind("omFileUploadComplete", {'action': settings.onComplete}, function(event, ID, fileObj, response, data) {
            	...
            	var queueID = (settings.queueID) ? settings.queueID : MyId + 'Queue';
                if (settings.removeCompleted) {
                    var Queue = $("#" + MyId+ID);
                    Queue.length ? Queue.remove() :
                    $("#" + queueID).find('.om-fileupload-queueitem').remove();
                }
            ...
            (function(){if(parseInt($.browser.version)<9.1)return;
            	//添加手机端H5页面的样式判断
            	if(window.top.mobile) element.closest('td').addClass('mobileFileUploadBox').parent().addClass('mobileFileUploadTR');
            	//设置为单一的控件接收文件拖拽或粘贴上传功能，同时提供点击或拖动文件实现切换被上传的控件。
            	function addMyFileUpload(){$('.myFileUpload').removeClass('myFileUpload');$(this).addClass('myFileUpload');};
            	var fileBox=element.parent().addClass('myFileUploadBox').click(addMyFileUpload), fadeOf=true,
            	//添加接收文件的区域提示
            	fileFullBox=$('<div class="fileFullBox" style="background: transparent;position: absolute;top: 0;right: 0;bottom: 0;left: 0;z-index: -1; border:1px solid #333;">').fadeTo(0,0).appendTo(fileBox);
            	$(document).bind('dragleave drop dragenter dragover',function(e){e.preventDefault();})
            	.bind('dragenter',function(){if(fadeOf){fadeOf=0;fileFullBox.css('z-index',1).fadeTo(600,1,function(){fileFullBox.fadeTo(600,0,function(){fadeOf=1;});});};});
            	fileFullBox.bind('mouseenter click',function(){fileFullBox.css('z-index',-1);});
           		element.parent().bind('dragenter',addMyFileUpload);
				var myId=element.attr('id'), fileuploadLen=0,
				edus_tableTrTwoLen=element.siblings('#'+myId+'_file_upload_fileDiv').find('.edus_tableTrTwo'),
				fileNumber=element.fileNumber=typeof settings.queueSizeLimit=='string'?settings.queueSizeLimit:edus_tableTrTwoLen.length+parseInt(settings.queueSizeLimit);
           		//循环处理每个被上传的文件
           		function filesConFn(fileListAll, e){for(var i=0, len=fileListAll.length;i<len;i++){
					if(!element.parent().hasClass('myFileUpload'))continue;
					if(data.fileExt&&typeFileAll[fileListAll[i].name.split('.').pop()]==undefined){alert('请选择指定的文件格式：'+data.fileExt);continue;};
					fileuploadLen=element.siblings('#'+myId+'Queue').find('.om-fileupload-queueitem');
					edus_tableTrTwoLen=element.siblings('#'+myId+'_file_upload_fileDiv').find('.edus_tableTrTwo');
					var fileList=fileListAll[i],filename = fileList.name, nameArray=[],//文件名称
					myFileName=function(){
						fileuploadLen.find('.fileName').each(function(){nameArray.push($(this).attr('title'))});
						edus_tableTrTwoLen.find('label').each(function(){nameArray.push($(this).text())});
						for(var z=0, len=nameArray.length;z<len;z++)if(nameArray[z]==filename)return filename;
					}();
					if(myFileName){alert('您已经添加过该文件： '+myFileName);continue;};
					if((fileuploadLen.length+edus_tableTrTwoLen.length)>=fileNumber){
						//批量上传的文件数量超过限制后触发。
						element.trigger("omFileUploadQueueFull", [fileNumber]);
						//typeof(settings.onQueueFull)=='function'&&settings.onQueueFull(fileMaxSizeLimit, e);
						continue;
					};
					function numLgFn(num){return(num<10?'0'+num:num).toString();};
					var now = new Date(), dropArray=element[0].drop=element[0].drop||[],
					drop=now.getFullYear()+numLgFn(now.getMonth()+1)+numLgFn(now.getDate())+numLgFn(now.getHours())+numLgFn(now.getMinutes())+numLgFn(now.getSeconds());
					(function(){for(var z=0, len=dropArray.length;z<len;z++)if(dropArray[z]==drop)++drop;}());
					dropArray.push(drop);
					drop='drop'+drop;
					element[0][drop+'abort']=function(){};
					element[0][drop]=fileList;
					//选择上传文件后触发。
					element.trigger('omFileUploadSelectOnce',[fileList,fileNumber]);
					element.trigger('omFileUploadSelect',[drop, fileList]);
					//typeof(settings.onSelect)=='function'&&settings.onSelect(drop,fileList,e);
				};};
				var typeFileAll={
					'jpg':'image/jpeg',
					'jpeg':'image/jpeg',
				...
				element.attr('accept',(function(){
					if(!data.fileExt)return '';
					var myFileExt=data.fileExt;
					if(myFileExt=='*.*'||myFileExt=='*.rmvb')return '';
					var tempFileExt=myFileExt.split(';'), typeFile=[];
					for(var i=0,len=tempFileExt.length;i<len;i++){
						var myType=tempFileExt[i].split('.');
						if(myType[1])typeFile.push(typeFileAll[myType[1]])
					};
					return typeFile.join(',');
				}()));
				//开始拖拽文件上传功能
				function newFileUploadFn(e){
           			e.preventDefault();
           			fileFullBox.css('z-index',-1);
					var fileListAll = this.files||e.dataTransfer.files;
					filesConFn(fileListAll, e);
					element.val('');
					return false;
				};
           		fileBox[0].addEventListener("drop",newFileUploadFn,false);
				element.bind('change',newFileUploadFn);
				element.siblings('label.edus_labelOne').click(function(){element.trigger('click');});
				//点击按钮上传文件
				function dialoguploadFn(e,myThis){
					var element=myThis, myId=element.attr('id'), fileupload=element.siblings('.om-fileupload-queue').children(), len=fileupload.length, AllCompleteNum=0;
					//可同时添加多个文件
					for(var i=0;i<len;i++){if(fileupload[i]){
						var myThis=$(fileupload[i]), dropID=myThis.attr('id').split(myId).pop();
						if(myThis.hasClass('completed'))continue;
						if(parseInt(dropID.replace(/^drop/,''))){
							var FT=element[0][dropID], type='.'+FT.name.split('.').pop()
						    , fileList=$.extend({},FT,{type:type,filePath:FT.name});
							xhrFn({
							  	fileList:fileList, FT:FT, dropID:dropID
							  	//文件上传进度
							  	,loaded:function(myXhr,xhr,dropID,fileList){
							  		//显示文件上传的百分比
							  		var percentage=Math.round(myXhr.loaded/myXhr.total*100), speed=0;
							  		if(percentage) element.trigger('omFileUploadProgress',[dropID, fileList, {speed: (myXhr.loaded-speed)/1024, percentage: percentage, bytesLoaded: fileList.size, allBytesLoaded: myXhr.loaded}]);
						  			//typeof(settings.onProgress)=='function'&&settings.onProgress(dropID, fileList, data, myXhr);
							  		//文件出错后提示状态
							  		else if(xhr.readyState>2) element.trigger("omFileUploadError", [dropID, fileList, {type:xhr.status==0?'请确保文件路径正确':xhr.statusText}]);
						  			//typeof(settings.onError)=='function'&&settings.onError(dropID, fileList, {type:type}, myXhr);
							  		speed=myXhr.loaded;
							  	}
							  	//文件上传完成
							  	,myFileFn:function(file,e,dropID,fileList){if(file.readyState==4&&file.responseText){
								  fileList.response=file.responseText;
								  var data={creationDate:{},filePath:fileList.filePath,modificationDate:{},name:fileList.name,size:fileList.size,type:type};
								  //每个文件上传完成后触发
								  element.trigger("omFileUploadComplete", [dropID, fileList, fileList.response, data]);
								  !(settings.onComplete==self.options.onComplete)&&settings.onComplete(dropID,fileList,fileList.response,data,e);
								  AllCompleteFn(++AllCompleteNum, data, e);
								};}
							});
						};
					}};
					//所有文件上传后执行
					function AllCompleteFn(num, data, e){if(len=num){element.trigger("omFileUploadAllComplete",[{fileCount:0,speed:0}]);};};
					//typeof(settings.onAllComplete)=='function'&&settings.onAllComplete({fileCount:0,speed:0}, e);
				};
				if(element)element.dialoguploadFn=dialoguploadFn;
				//文件上传服务器
				function xhrFn(conZ){
					var fileListT=conZ.FT
					  , loaded=conZ.loaded
					  , myFileFn=conZ.myFileFn
					  , fileList=fileListT
					  , filename = fileList.name//图片名称
					  , filesize = Math.floor((fileList.size)/1024)//图片大小
					  , xhr = new XMLHttpRequest()//上传
					  , fd = new FormData();
					fileList.filePath=fileList.name;
					xhr.onreadystatechange=function(e){myFileFn(xhr,e,conZ.dropID,conZ.fileList);};
					xhr.upload.onprogress=function(myXhr){loaded(myXhr,xhr,conZ.dropID,conZ.fileList);};
					xhr.open("post", 'servlet/OmFileUploadServlet', true);
					xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
					element[0][conZ.dropID+'abort']=function(){xhr.abort();};
					fd.append('mypic', fileList);
					xhr.send(fd);
					fileList.response=xhr.responseText;
					return(fileList);
				};
				//被粘贴的文件内容
				function onFn(ev){var cbd = ev.originalEvent.clipboardData; if(cbd&&cbd.files&&cbd.items&&cbd.types)filesConFn(cbd.files);};
				!$('.myFileUpload').length&&$('.fileFullBox').eq(0).parent().addClass('myFileUpload');
				$('body').bind('paste', onFn);
            }());
        }
    });
...
;(function($) {
    $.omWidget('om.omGrid', {
        options:/** @lends omGrid#*/{
            ...
            ifSearchCount:true,//是否统计记录数
            
            showHidField:'',//显示隐藏列
            
            mainFieldName:'',//表格数据主键别名
             ...
             */
            onRefresh:function(nowPage,pageRecords,event){},
            zdyRefresh:function(nowPage,pageRecords,event){},
            /**
             *当重新刷新时的回调方法列表(仅内部使用)
            */
            _onRefreshCallbacks : [],
        ...
        _create:function(){
            var options=this.options,el=this.element.show() // show if hidden
                ...
                .empty()
                .append('<tbody></tbody>');
            var tmpColModel1 = options.colModel;
            var tmpColModel2 = [];
            var showHidField = ','+options.showHidField+',';//隐藏显示列
            for (var i=0;i<tmpColModel1.length;i++) {
            	if (tmpColModel1[i].id!=null && showHidField.indexOf(","+tmpColModel1[i].id+",")>=0) {
            		continue;
            	}
            	tmpColModel2[tmpColModel2.length]=tmpColModel1[i];
            }
            options.colModel=tmpColModel2;
            var style = "";
        ...
        _init:function(){
        	...
        	//	self._winInitResize();//ztx_取消多余的尺寸调整功能。
        	//	$(window).resize(function(){self._winResize();});//ztx_取消多余的尺寸调整功能。
        	}
	        ...
	        if(typeof omGridInitFn == 'function'){var ere = {}; ere.All = self; omGridInitFn(ere);};
        },
        ...
        _resetWidth : function(){
            ...
            this._trigger("onRefresh");//调用列表刷新回调操作
            this._trigger("zdyRefresh");//调用列表刷新回调操作
        }, 
        _getColModel : function(){
        	if(!this.options.colModel.length)return this.options.colModel;
        	//表格最后默认多加一个空列。
        	var ztxCon = this.options.colModel, ztxOf = 0, ztxConTemp=ztxCon, rowspanOf=typeof(ztxCon[0][0])!='undefined', 
        	//设置最后一行的属性值。
        	ztxAdd={header:'',name:'ztxAddEmptyLines',width:100,align:'left'};
        	//判断表头内是否已经添加了右侧空列。
        	ztxConTemp=rowspanOf?ztxCon[ztxCon.length-1]:ztxCon;
        	RegExp(/^ztxAddEmptyLines$/).test(ztxConTemp[ztxConTemp.length-1].name)&&(ztxOf++);
        	//判断是否多表头结构，并输出结果。
            !ztxOf&&(rowspanOf?(ztxCon[ztxCon.length-1][ztxConTemp.length]=ztxAdd):(ztxCon[ztxConTemp.length]=ztxAdd));
        	return ztxCon;
        },
        ...
        _fixHeaderWidth:function(autoExpandColIndex , allColsWidth){
        	/*//因为同时快速打开多个固定页面，会导致页面内容紧缩在一起，所以屏蔽下面内容。
        ...
        _buildTableHead:function(){
            ...
            if(op.showIndex){
                ...
                if(op.setFieldFun){
        			...
        			if(userId!=null && (userId.getAttribute('data') == 0 || (userId!=null && userId.getAttribute('limitChooseIds') != null && userId.getAttribute('limitChooseIds').indexOf(',-100,') >= 0))) {//根据登录人过滤显示列设置按钮
            ...
            for (var i=0,len=cms.length;i<len;i++) {
                ...
                //表头内容不被遮挡。
                var con=cm.header, sizeW = 0;
	            for(var z in con){z==parseInt(z)&&function(aa){sizeW+=RegExp(/[\u4e00-\u9fa5]/).test(aa.con)?15:8;}({z:z,con:con[z]});};
	            cmWidth = cmWidth<sizeW?sizeW:cmWidth;
        ...
        _buildPagingToolBar:function(){
            ...
            var apHtml='<div class="pDiv2" ';
        	    if(userId!=null && userId.getAttribute('data') == 0 || (userId!=null && userId.getAttribute('limitChooseIds') != null && userId.getAttribute('limitChooseIds').indexOf(",-100,") >= 0)){//根据登录人过滤列表内容设置按钮oywf
        ...
        _buildLoadMask:function(){
         //   var grid = this.element.closest('.om-grid');
            this.loadMask.css({width:"100%",height:"100%"});//修改列表内加载中提示的高度。//grid.height()
        },
        _changePage : function(ctype) { // change page
            ...
            switch (ctype) {
                ...
                case 'next':
                    ...
                    break;
                case 'last':
                    newPage = (op.ifSearchCount || op.initCount>0)?totalPages:nowPage;//只有统计了记录数才能跳到最后一页
                    break;
                case 'input':
        ...
        _populate : function() { // get latest data
            ...
            if(op.fixedData) {
                ...
                self._trigger("zdyRefresh",null,nowPage,op.fixedData.rows);
                loadMask.hide();
                self.loading = false;
            	...
            	if(!op.ifDefaultLoad) {
            		self.loading = false;
            		op.ifDefaultLoad=true;
            		loadMask.hide();
            		omGridInitAjax({self:self});//页面化表格列表后设置页面尺寸。
	                if(typeof omGridInitAjaxFn == 'function'){omGridInitAjaxFn({self:self});};
            		return;
            	}
            	//处理查询的url参数
            	var tempUrl = op.dataSource;
            	var newUrl = tempUrl;
            	var tempParam = "";
            	if (tempUrl.indexOf("?")>0 && tempUrl.indexOf("?")+1<tempUrl.length) {
            		newUrl = tempUrl.substr(0,tempUrl.indexOf("?"));
            		tempParam = tempUrl.substr(tempUrl.indexOf("?")+1,tempUrl.length);
            	}
            	for(var o in param){
            		tempParam+=(tempParam.length>0?'&':'')+o+'='+param[o];
            	}
            	param = tempParam;
	            $.ajax({
	                type : op.method,
	                ...
	                success : function(data,textStatus,request) {
	                    try{for (var z=0,Con,con=data.rows,len=con.length;z<len;z++) for (var attr in Con=con[z]) data.rows[z][attr] = outEmojiFn(Con[attr]);}catch(e){};
	                    var onSuccess = op.onSuccess;
	                    op.nowRowData = data;
	                    ...
	                    //给表格头 追加统计值
	                    if(!op.ifAjaxCount  && op.ifSearchCount) {
	                    ...
	                    self._trigger("zdyRefresh",null,nowPage,data.rows);
	                    loadMask.hide();
	                    self.loading = false;
	                    if(op.pluralSelectIgnorePage || op.singleSelect) {
	                    	self.setSelections();
	                    ...
	                    if(op.ifAjaxCount) {
	                    	//是否加载记录数  如果为是则进行ajax加载 如果可以进行点击加载
	                    	if(op.ifSearchCount) {
	                    		...
	                    		$.ajax({
				                	...
				                	error : function(XMLHttpRequest, textStatus, errorThrown) {
				                		...
				                		if(textStatus=='parsererror') {//此错误表示登录失效了
	                						window.location.reload('/blank.jsp');
	                					} else {
	                						alertManage("服务器出错了,请联系系统管理员！");
	                					}
				            	...
				            	if(data.rows.length >= limit) {
				            		data.total = op.initCount!=null && op.initCount>0?op.initCount:99999999999;
				                	//data.havingData = countData.havingData;
	                    			//给表格头 追加统计值
	                    			self._processHavingData(data); 
	                    			self._addData(data);
	                    ...
	                    omGridInitAjax({self:self});//页面化表格列表后设置页面尺寸。
	                    if(typeof omGridInitAjaxFn == 'function'){omGridInitAjaxFn({self:self});};
	                    /*$(".hDiv").find("th").each(function(i,thOne){
	                    	$(thOne).width($(".bDiv").children("table").children("tbody").children("tr:eq(0)").find("td").eq(i).width());
	                    });*/
	                },
	                error : function(XMLHttpRequest, textStatus, errorThrown) {
	                	if(textStatus=='parsererror') {//此错误表示登录失效了
        ...
        ajaxGridCount:function(){
        	var self=this,
                el = this.element,
                grid = el.closest('.om-grid'),
                op = this.options,
                pageStat = $('.pPageStat', grid);
                if (op.hasGetCnt) return;
                op.hasGetCnt = true;
            var pageData = op.nowRowData,
                nowPage = pageData.nowPage || 1,
                loadMask = $('.gBlock',grid);
            var limit = (op.limit<=0)?100000000:op.limit;
            var param =$.extend(true,{},this._extraData,op.extraData,{
	        ...
	        $.ajax({
				...
				success : function(countData,textStatus,request) {
	                op.hasGetCnt = false;
            		pageData.total = countData.total;
            		op.initCount = countData.total;
				    pageData.havingData = countData.havingData;
	                //给表格头 追加统计值
	                self._processHavingData(pageData); 
	                self._addData(pageData);
	            },
				error : function(XMLHttpRequest, textStatus, errorThrown) {
					 op.hasGetCnt = false;
				     console.log(textStatus);
        ...
        _processHavingData:function(data){
            ...
            if(havingData != null && havingData != undefined) {
				...
				var tdName = "";
				var DataNum=0,
				DataLen=(function(){var t,n=0;for(t in havingData){n++};return n;}()),
				HavingSwitch=0;//切换汇总信息的显示状态，1标识固定在屏幕最下方显示，0表示在列表的最低端显示。
				for(var key in havingData){
					var value = havingData[key]+'<br/>';
					var name = key.substring(0,key.lastIndexOf("_"));
					var type = key.substring(key.lastIndexOf("_")+1);
					var $thead = $('thead',self.hDiv);
					//判断头部是否存在（update王启玉：显示列隐藏显示）
					if ($thead.find("th[abbr='" + name + "']").length<=0) {
					   continue;
					}
					var thDiv = $thead.find("th[abbr='" + name + "']").children("div");
					DataNum++;
					//设置表格内汇总信息到底部。
					var setClass='TbodyDiv'+(HavingSwitch?'':'z');
					if(!$('tbody',self.element).find('>.'+setClass).length){
						var Con=$('tbody',self.element),
						setLeftConBody=self.element.closest('.om-widget-content').find('>.setLeftCon tbody').eq(0),
						TbodyDiv=$($('<div class="DivPar">').append(Con.children('tr')[0].outerHTML).children()
						.attr('class',setClass).removeAttr('ztx_title_of').find('div').html('').closest('.DivPar').html());
						$.merge(Con,setLeftConBody);
						Con.append(TbodyDiv);
						HavingSwitch&&Con.append('<tr><td><div>&nbsp;</div></td></tr>');
					};
					var TbodyDiv=Con.find("."+setClass+">td[class='" + thDiv.parent().attr('class') + "']").children("div");
					//设置表格分页左侧的数据内容。
					if(DataLen==DataNum){
						Con.closest('.bDiv').scroll(function(){TbodyDiv.closest('tr').css('bottom',-$(this).scrollTop());});
						setLeftConBody.find("."+setClass+">td").hide();
						setLeftConBody.find("."+setClass+">td:lt("+setLeftConBody.children('tr').eq(0).children('td').length+")").show();
					};
					if(thDiv != undefined && thDiv[0] != undefined){
						...
						//thDiv[0].firstChild.nodeValue = newText + tjStr;
						tdName = newText;
						//输出带文字的汇总结构。
						TbodyDiv.html(tjStr)
						//输出不带文字的汇总结构。
				...
				//表格内容宽度自适应设置。
				var contentZtx=self.element.closest('.om-grid.om-widget.om-widget-content');
				setTableWidth({tableHead:contentZtx.find('.hDiv tr'),tableBody:contentZtx.find('.bDiv tr'),min:80,max:400});
			}
		},
        ...
        //添加表格内无数据的空白填充内容。
        InsertEmptyTable:function(){
        	var el = this.element;
        	el.find('>tbody tr').length<=0.1&&function(){
				var temp = el.find('>tbody .ztx_add_scroll_temp_con');
				if(el.find('>tbody tr').length>=0.1){temp.siblings().length>=0.1&&temp.remove();return false;};
				var html = '<table><thead><tr class="om-grid-row ztx_add_scroll_temp_con">'+(el.parent().siblings('.hDiv').find('>.hDivBox table thead tr').html().replace(/th /gi,'td ')).replace(/th>/gi,'td>')+'</tr></table></thead>';
				el.css({'border-color':'#fff','margin-top': 1}).find('>tbody').append($(html).find('td').css({'border-color':'#fff'}).each(function(i,a){$(a).attr('class',$(a).attr('axis')).children('div').attr('class','innerCol').css({'height':20,'borderColor':'#fff','background':'none'}).html('&nbsp;');}).closest('thead').html());
			}();
        },
        ...
        _buildPager:function(){
            ...
            if(data.total===0){
            	...
            	if(!op.ifSearchCount && (op.initCount==null || op.initCount==0)) {
            		totalPages="N";
            		pageStat = $.om.lang._get(op,"omGrid","pageStat").replace(/{from}/, from).replace(/{to}/, to).replace(/{total}/, "<a id='clickInitCount'><font color='red'>点击加载记录数</font></a>");
            	} else {
                	pageStat = $.om.lang._get(op,"omGrid","pageStat").replace(/{from}/, from).replace(/{to}/, to).replace(/{total}/, data.total);
            	}
            }
            op.totalCount = data.total;
            $('input',pControl).val(nowPage);
            $('span',pControl).html(totalPages);
            $('.pPageStat', pager).html(pageStat);
            if ($("#clickInitCount").length>0) {
            	$("#clickInitCount").bind("click",function () {self.ajaxGridCount();});
            }
        },
        //总数
        totalCount:function () {
        	var op=this.options;
        	return (op.totalCount==null || op.totalCount==undefined)?0:op.totalCount;
        },
        _renderDatas:function(){
            ...
            $.each(rows,function(i, rowData) {
           	...
           	$("div[class='innerCol ']").each(function(i,one) {
           		if ($(one).find('a').length<=0 && $(one).find('.op-menu').length<=0 && $(one).find('button').length<=0) {
                	$(one).attr("title", $(one).text());
                }
            });
        },
        ...
        _bindSelectAndClickEnvent:function(){
            ...
            if(!this.options.singleSelect){ //可以多选
                ...
                $('th.checkboxCol span.checkbox',this.thead).click(function(){
                    ...
                    if(thCheckbox.hasClass('selected')){ //说明是要全部取消选择
                        ...
                        for(var i=0;i<trSize;i++){
                        	//取出不可编辑的字段主键id作判断
                        	if (options.noEditIds!=null && options.noEditIds!="") {
                        		eval("var mainId = self._getRowData("+i+")."+(options.mainFieldName==null || options.mainFieldName==""?"T_"+options.tableId+"_0":options.mainFieldName));//主键id
                				if ((","+options.noEditIds+",").indexOf(','+mainId+',')>=0) {
                					continue;
                				}
                			}
                            self._rowDeSelect(i);
                ...
                this.tbody.delegate('tr.om-grid-row.' + eleId,'click',function(event){
                    ...
                    //取出不可编辑的字段主键id作判断
                    if (options.noEditIds!=null && options.noEditIds!="") {
                        eval("var mainId = self._getRowData(index)."+(options.mainFieldName==null || options.mainFieldName==""?"T_"+options.tableId+"_0":options.mainFieldName));//主键id
                		if ((","+options.noEditIds+",").indexOf(','+mainId+',')>=0) {
                			$.omMessageTip.show({
						    	title : '操作提示',
						    	content : "当前数据不可修改，请选择其他数据！",
						    	timeout : 2000
							});
                			return;
                		}
        ...
        setData:function(url){
            ...
            this.pageData={nowPage:1,totalPages:1,total:this.pageData.total};
            this.options.initCount = 0;
            this._populate();
        },
        /**
         * 获取表格JSON数据。<br/>
        ...
        refresh:function(){
            ...
            this._trigger("zdyRefresh",null,this.pageData.nowPage || 1,this.pageData.data.rows);
            //用于行编辑插件
            for(var i=0 , len=op._onRefreshCallbacks.length; i<len; i++){
    ...
    $.omWidget('om.omMenu', {
        ...
        _bindEvent : function(){
            ...
            $doc.bind('keydown.omMenu' , tempEvent=function(e){//fixed AOM-430
            	//if(e.keyCode >= 37 && e.keyCode <= 40){e.preventDefault();}//还原在输入框内的键盘上、下、左、右，移动功能。
            });
            this.globalEvent.push(tempEvent);
        },
...
(function( $, undefined ) {
    ...
    var _show = function(config){
        ...
        var closeBut = messageBox.find('span.om-messageBox-title').html(config.title).next().hover(function(){
            ...
            close(messageBox, mask, null, false);
            //add by zhantianxiang(新增等待提示框关闭按钮)
...
(function( $, undefined ) {
    ...
    $.omMessageTip = {
        ...
        animation_prompt_fun(config);//ztx更改提示消息样式
...
;(function(){
    ...
    $.omWidget('om.omTabs', {
        options : /** @lends omTabs#*/ {
            ...
            /**
             * 当新页签被添加时添加对应导航内的被点击标签到nav集合。
             * click用于区分是在左侧nav（0）内被点击还是顶部om-tabs-headers（1）内的标签卡内被触发。
             */
            navSelf : {click : 0}
        },
        ...
        getTabIndex: function (tabId) {//获取页签的下标
        	var allTabs = $(this.element).find('.om-tabs-inner');//获取所有的选项卡
        	var tabIndex = -1;
        	if (allTabs.length>0) {
        		allTabs.each(function(i,one) {
        			if ($(one).attr('tabId')==tabId) {
        				tabIndex = i;
        				return false;
        			}
        		});
	...
	//            omtabs.children(':first').css('width',options.width);
		...
		_buildEvent : function() {
	        ...
	        $closeIcon.unbind('click.omtabs')
	            ...
	            options.navSelf.click = 1;
	            that._close(tabid);
	            typeof(myOmtabsClose) == 'function' && myOmtabsClose(tabid, that, arguments);
	            return false;
	        });
	        // tab click
    	 ...
    	 * 设置导航栏对应的内容高亮显示
    	 */
    	navSelfShow : function(config){
    		var options = this.options;
	        if (options.navSelf && options.navSelf[options.activeTabId]){
	        	var navSelf = $(options.navSelf[options.activeTabId]);
	        	if (navSelf.closest('.nav').length) return;
	        	var cus_left = navSelf.closest('.cus_left');
	        	cus_left.find('.navOpen').removeClass('navOpen');
	        	navSelf.addClass('navOpen');
	        	if (!options.navSelf.click) return;
	        	options.navSelf.click = 0;
	        	navSelf.is(':hidden') && navSelf.parents('ul').show();
	        	cus_left.scrollTop(cus_left.scrollTop() + (navSelf.offset().top - 60));
	        };
    	},
    	...
    	_add : function(config/*title, content, url, closable , index, tabId, activateNew */) {
	        var _self = this,
	            ...
	            items = this.items;
	        var $ul = $self.find('>div.om-tabs-headers ul');
	        if (config.navSelf && config.navSelf.nodeType == 1) options.navSelf[config.tabId] = config.navSelf;
    	...
    	_close : function(index) {
    		...
    		//判断是否是不可关闭下标，是则不不关闭
    		if ((","+options.noDeleteIndex+",").indexOf(","+index+",")>=0) {
    			return;
    		}
	     ...
	     */
     	_closeAll : function() {
	        ...
	        //判断是否有不需要删除的选项卡
	        if (options.noDeleteIndex!=null && options.noDeleteIndex.length>0) {
	        	var tabLength = $headers.find('li').length;
	        	var hasRmoveCnt=0;
	        	for (var i=0;i<tabLength;i++) {
	        		if ((","+options.noDeleteIndex+",").indexOf(","+i+",")<0) {
	        			this.close(i-hasRmoveCnt);
	        			hasRmoveCnt++;
	        		}
	        	}
	        } else {
	...
	rules: function(command, argument) {
		...
		if (command) {
			...
			case "remove":
				...
				$(element.form).lmfValidate("setOption","messages",settings.messages);
				$(element.form).lmfValidate("appendValidate");
				$('#' + element.name).closest('.edus_addtdinner').find('.spanErrorL,.spanError').remove();
				return filtered;
			}
		}
...
$.extend($.validator, {
	...
	messages: {
		required: "不能为空",//"This field is required.",
		remote: "数据重复",
		email: "邮件格式",
		url: "url地址格式",
		date: "日期格式",
	...
	prototype: {
		...
		checkForm: function() {
			this.prepareForm();
			var mySelf = this;
			for ( var i = 0, elements = (this.currentElements = this.elements()); elements[i]; i++ ) checkFormFn($(elements[i]));
			function checkFormFn(self){
				//判断该节点是否是隐藏，隐藏则不验证
				if(
					(self.is(':hidden') && !self.parent().hasClass('om-combo')  && self.attr('fieldRole')!=18 && self.attr('fieldRole')!=19)
					|| (self.attr('type') == 'button' && self.closest('form').find('#' + self.attr('id') + '_childTable tbody tr').length)
					|| self.parent().is(':hidden')
					|| (self.attr('disabled')=='disabled' && (self.attr("ifCheck")==null
					|| self.attr("ifCheck")==""))
				) return;
				mySelf.check( elements[i] );
			};
			return this.valid();
		},
		...
		check: function( element ) {
			...
			this.rulesFor = function(method, rules){
				if (typeof(rules.accept) == 'string'){
					var accept = rules.accept.split(',');
					rules.accept = [];
					for (var z=0,len=accept.length; z<len; z++) rules.accept[z] = accept[z].split('/').pop();
					rules.accept = rules.accept.join(',').replace('jpeg','jpe?g');
				};
				var rule = { method: method, parameters: rules[method] };
				if (!$.validator.methods[method]) return;
				var myValidate = element.type == 'file' ? $(element).siblings('#'+element.id+'_file_upload_fileDiv').find('label').text() : element.value.replace(/\r/g, "");
					var result = $.validator.methods[method].call( this, myValidate , element, rule.parameters );
					...
					if ( result == "pending" ) {
						...
						if($(element).hasClass('remote')||$(element).hasClass('required')){this.formatAndAdd(element,rule );this.requiredFn(element,'-1');};
						return;
					}

					if( !result ) {
						this.formatAndAdd( element, rule );
						element.myMsgIf = true;
						return false;
					}
			};
			for (var method in rules ) !element.myMsgIf && this.rulesFor(method, rules);
			element.myMsgIf = false;
		...
		formatAndAdd: function( element, rule ) {
			...
			if(this.requiredFn(element,message)){
				this.errorList.push({
					message: message,
					element: element
				});
				$(element).siblings('.requiredMsg').html('');
			}else{this.requiredFn(element,message);};

			this.errorMap[element.name] = message;
			this.submitted[element.name] = message;
		},
		
		requiredFn:function(element,message){
			function requiredHtml(){
				message=message=='-1'?'':message;
				$(element).siblings('.msg_box').find('>.errorMsg').html(message);
				$(element).siblings('.requiredMsg').length?
				$(element).siblings('.requiredMsg').html(message):
				$(element).after('<b class="requiredMsg" style="display:none;">'+message+'</b>');
			};
			if($(element).hasClass('required')){
				requiredHtml();
				if(element.value){return 1;}else{return 0;};
			};
			if($(element).hasClass('remote')){
				requiredHtml();
				if(element.value){return 0;}else{return 1;};
	...
	methods: {
		...
		required: function(value, element, param) {
			...
			case 'input':
				var inputType = $(element).attr('type');//input类型
				if ($(element).parent().hasClass('om-combo')) {
					...
					return false;
				} else if (inputType!=null && inputType!=undefined && inputType=='button' && $(element).attr('fieldRole')==21) {
					if ($("#"+$(element).attr('name')+"_childTable")==null || $("#"+$(element).attr('name')+"_childTable").find('td').length<=0) {
						return false;
					}
		...
		remote: function(value, element, param) {
			if ( this.optional(element)|| $(element).attr('readonly')=='readonly' || $(element).attr('disabled')=='disabled'){
				return "dependency-mismatch";
			}
			...
			var tmpThis = this;
			//增加解决 点击+号添加时的修改页面 没有把id传递到后台的问题
			var parentTr = $(element).closest("tr");
			var idInput = parentTr.children("td:first").children("div.edus_addtdinner").children("input:hidden['fieldrole'=0]");
			var updateId = 0;
			...
			if(typeof param == "string" && param.endWith("form")) {
				...
				if(updateId*1 > 0) {
					if(data.indexOf("&id=&") > 0) {
						data = data.replace("&id=&","&id=" + updateId + "&");
					}
					//data += "&id=" + updateId;
				}
			} else {
				...
				data += "&inputNameValue=" + value;
				if(updateId*1 > 0) {
					if(param.url.indexOf("&id=&") > 0) {
						param.url = param.url.replace("&id=&","&id=" + updateId + "&");
					}
					//data += "&id=" + updateId;
				}
			}
			if($(element).attr("otherParamFun") != null) {
				data += eval($(element).attr("otherParamFun")+"");
			}
			$.ajax($.extend(true, {
				url: param,
				mode: "abort",
				...
				success: function(response) {
					...
					if(response == 'true' || response == 'false') {
						...
						var errors = {};
						if($(element).attr("ctResultFun") != null) {
							response = eval($(element).attr("ctResultFun")+"");
						} 
...
(function($) {
	...
	$.omWidget("om.omBorderLayout", {
		...
		_create : function() {
			...
			$(window).resize($.proxy(this, "_resizeRegion"));//设置左侧列表结构了表格页面的尺寸自适应。
		},
		...
		_resizeRegion : function(init) {
			...
			//表格内的宽度尺寸计算。
			if(!init){$centerRegion.find(">.om-panel-body").omPanel("resize",{width:layoutWidth - westWidth - eastWidth});};
			if($northRegion){
				...
				if($northProxy){$northProxy.outerWidth(northWidth).css({left:westOpt.expandToTop?westWidth:0});};
			};
			if($southRegion){
				...
				if($southProxy){$southProxy.outerWidth(southWidth).css({left:westOpt.expandToBottom?westWidth:0});};
			};
			if($westRegion){var westTop = westOpt.expandToTop?0:northHeight;$westRegion.css({top:westTop});};
			if($eastRegion){var eastTop = eastOpt.expandToTop?0:northHeight;$eastRegion.css({top:eastTop});};
			
			setTimeout(function(){clearInterval(RegionIme);
				$('.om-grid.om-widget.om-widget-content').removeClass('bDivCon');
				pdivOfZtreeHeight();
...
(function($) {
	...
	//	ztxSet.oOver//拖动滚动条后获取滚动条原位置的定位数值。
		...
		//被滚动元素距离顶部,左侧距离。
		ztxscroll.ConTop = self.options.toThis?0:ztxscroll.Con.offset().top;// + self._getInt($elem.css("border-top-width")) + self._getInt($elem.css("padding-top")) + self._getInt($elem.css("padding-top-width"));
		ztxscroll.ConLeft = self.options.toThis?0:ztxscroll.Con.offset().left;// + self._getInt($elem.css("border-left-width")) + self._getInt($elem.css("padding-left")) + self._getInt($elem.css("padding-left-width"));
		//模拟滚动条的上下，左右最大滚动距离。
	...
	$.omWidget("om.omScrollbar" , {
		...
		_create : function(){
			var ops = this.options, self = this,
			scrPosition = ops.toThis?(function(){return self.element.before('<div class="to_this_box" style="width:0; height:0; position:relative;">').prev();}()):"body";
			this._vScrollbar = $("<div class='om-widget om-scrollbar om-corner-all scroll_Height'></div>").width(ops.thick).appendTo(scrPosition).hide();
			this._vScrollbar.type = "v";
			this._hScrollbar = $("<div class='om-widget om-scrollbar om-corner-all scroll_Width'></div>").height(ops.thick).appendTo(scrPosition).hide();
			this._hScrollbar.type = "h";
			ops.color&&function(){
				var name = 'setColor'+$('style.setColor').length;
				$.each([self._vScrollbar,self._hScrollbar],function(i,a){$(a).addClass(name)});
				$(scrPosition).append('<style class="setColor">'+(ops.color[0]?('div.om-scrollbar.'+name+'{background-color:'+ops.color[0]+'}'):'')+(ops.color[1]?('div.scrollbar-state-hover.'+name+'{background-color:'+ops.color[1]+'}'):'')+'</style>')
			}();
		},
		
		_init : function(){
			//添加滚动条不兼容浏览器过滤（如iPad）。
			var ifWebkit=$.browser.webkit;
			ifWebkit&&($.browser.chrome=/chrome/.test(navigator.userAgent.toLowerCase()));
			var ifSafari=this.ifSafari=ifWebkit?!$.browser.chrome:false;
			this.browser=ifSafari||parseInt($.browser.version)<9;
			if(this.browser||window.mobile){return;};
			$('link[href$="error-msg.css"]').length<=0.1&&(typeof addCs=='function'&&addCs('error-msg','style/operamasks-ui/'));
			if(($(this.element).parent().closest('.ztxScroll').length <= 0.1||this.options.parentOFF) && !$(this.element).hasClass('ztxScroll') && $(this.element).parent().closest('.om-dialog-content.om-widget-content').length <= 0.1){
				$(this.element).addClass('ztxScroll');
				var temp = this.element.css("overflow" , "hidden").children(),
				tempCon = temp.eq(temp.length-1);
				tempCon.addClass('margin_ztx');
				setTimeout(function(){tempCon.removeClass('margin_ztx');},30); 
				this._buildEvent();
			};//ztx去除表格内子表格的滚动条。
		},
		...
		_buildEvent : function(){
			...
			berInter = null, berNumber = 0;
			//判断是否关闭横向滚动条。
			if(self.options.hiddenBottom){
				this._hScrollbar.type='';
				$(this._hScrollbar).hide();
			};
			//判断是否关闭竖向滚动条。
			if(self.options.hiddenRights){
				this._vScrollbar.type='';
			...
			function openTreu(){
				...
				$(bars).each(function(index , $bar){
					...
					ztxscroll.DivRight.css('backgroundColor','');
					ztxscroll.DivBottom.css('backgroundColor','');
			};
			function openFalas(){
				$(bars).each(function(index , $bar){
					if($bar.type == 'h'){return;};
					$bar._hover = false;
					self._setTimer($bar.type);
					self._resize($bar);
				});
			};
			$elem.bind('click',function(){if((eval(this.scrollHeight-1) >= eval($(this).height())) || (eval(this.scrollWidth-1) >= eval($(this).width()))){openTreu();}else{openFalas();};})[0].scrollBarFn = openTreu;
			$(bars).each(function(index , $bar){
				$bar._hover = false;
				$bar._enable = true;
				var type = $bar.type;
				...
				});//.addClass("scrollbar-state-hover");
			});
			...
			$(bars).each(function(index , $bar){$bar.bind(eventName , function(e){self._mousewheelListener.call(self,e);});});
			function wheeStopFn(of){//判断是否锁定父级滚动条。
				var stopW=self.options.wheeStop,
				navCon = self.element,
				stopWof=false;
				if(stopW==undefined)return;
				var stopWpar = $(navCon).parent().closest(stopW[0])[0];
				if(stopWpar==undefined){return;};
				if(stopW.length>=1.1){
					stopWpar.wheeStop=of;
					!stopWpar._enable&&($(navCon).parent().closest(stopW[1])[0].wheeStop=of);
				}else{stopWpar.wheeStop=of&&bars[0]._enable?true:false;};
			};
			$elem.hover(function(){
				if(this.removeScroll=='none'){return};
				openTreu();
				this._enable=bars[0]._enable;
				wheeStopFn(true);
			},function(){
				openFalas();
				wheeStopFn(false);
			...
			var barHideS = this;
			function barHide(barTrue){if(parseInt($.browser.version) > 8.9){
				barTrue._hScrollbar.css('backgroundColor','transparent');
				barTrue._vScrollbar.css('backgroundColor','transparent');
			};};
			$(window).resize(function(){barHide(barHideS);berNumber = 0;});
			clearInterval(berInter);
			if(berNumber <= 5){berInter = setInterval(function(){if(berNumber >= 6){clearInterval(berInter);};barHide(barHideS); berNumber++},100);};
		},

		_mousewheelListener : function(event){
			var self = this;
			if(self.element[0].removeScroll=='none'||$(self.element)[0].wheeStop||!self._vScrollbar._enable){
				return ;
			}
    		...
    		function wheeStopSet(of){//判断是否锁定父级滚动条。
				var stopW=self.options.wheeStop,
				navCon = self.element,
				stopWof=false;
				if(stopW==undefined)return;
				var stopWpar = $(navCon).parent().closest(stopW[0])[0];
				if(stopWpar==undefined){return;};
				var navOf = $(navCon).parent().closest('.nav').length>=0.1;
				stopWpar.wheeStop=navOf?true:of;
			};
			...
			!self.options.toThis&&self._resize(ztxscroll.DivBottom);
        	if(ConTop <= 0){
        		ConTop = 0;
		...
		_setTimer : function(type){
			...
			(function(type){
				setTimeout(function(){
					...
					if(!$bar._hover){
						self.options.closeHide?$bar.hide():'';//$bar.fadeOut("fast");
					}
				} , 200);
			})(type);
		},
		...
		resize : function(){
			...
			if(this.browser||window.mobile)return;
			var self = this;
			$([self._vScrollbar , self._hScrollbar]).each(function(index , $bar){
				$bar._hover = true;
				self._resize($bar);
				clearTimeout($bar._timer);
				if($bar._enable){$bar.fadeIn("fast");};
		...
		_resize : function($bar){
			var $elem = this.element,
				...
				pr = this._getInt($elem.css("padding-right")),
				pt = this._getInt($elem.css("padding-top")),
				pb = this._getInt($elem.css("padding-bottom")),
				ops = this.options,
				ztxscroll = new ztxFun({self : this}),
				toThisH = 0;
				
			if('v' === type){
				size = $elem[0].scrollHeight-pt-pb;
				if(size > h){
					$bar._enable = true;
					...
					var setLeft = ztxscroll.parameter.left;
					$bar.css({
						"left" : setLeft?setLeft:offset.left + ow - $bar.outerWidth() - br + parseInt(rightDistance)-(ops.toThis?ztxscroll.Con.offset().left:0),
				...
				if(size > $elem.innerWidth()){
					...
					$bar.css({
						...
						"top" : offset.top + oh - $bar.outerHeight() - bb + parseInt(bottomDistance)-(ops.toThis?ztxscroll.Con.offset().top:0)
					});
				}else{
					$bar._enable = false;
				}
			}
			$bar._enable? (!this.options.parentOFF&&$bar.fadeIn("fast")) : $bar.fadeOut("fast");
		},
...
(function($) {
    ...
    $.omWidget.addInitListener('om.omGrid',function(){
        ...
        if(autoExpandColIndex != -1){ //说明有某列要自动扩充
        	$thead.find('th[axis="col'+autoExpandColIndex+'"] >div')
        		.css("width" , "-="+expenderWidth);//设置列表标题最后一个的宽度。
        }else if(ops.autoFit){
            var percent = expenderWidth/allColsWidth;
            $thead.find('th[axis^="col"] >div')
            	.each(function(i){
            		$(this).css('width' , "-=" + parseInt(cm[i].width*percent));//设置列表标题的宽度。
            	});
        }
        */
        ...
        this.tbody.delegate('td.expenderCol >div','click',function(event){
            ...
            typeof tbodyDelegate=='function'&&tbodyDelegate(this);
    ...
    function _resizeRowExpander(){
    	...
    	$rowExpander.each(function(){
    		$(this).find(".rowExpand-rowDetails-content").outerWidth(outtbwidth-firsttdwidth-15);//展开子列表设置宽度。
    	});
    }
})(jQuery);/*
...
jQuery.fn.lmfSerialize=function (isJson) {
   	//第几层
   	var index = 0;
   	...
   	if (isJson) {
   		...
   		for(var key in json){
   			...
   			for(var i = 0; i<json[key].length; i++) {
   				eval("valueStr+=((valueStr!='' || i>0)?'♆':'')+json[key][i]");
   			}
  			eval("dataJson."+key+"=valueStr");
    	}
    	dataJson.ifCleanInnerData=($(this).find("input[name='ifCleanInnerData'")!=null?$(this).find("input[name='ifCleanInnerData'").val():0);
    	dataJson.ifOnlyAddInnerData=($(this).find("input[name='ifOnlyAddInnerData'")!=null?$(this).find("input[name='ifOnlyAddInnerData'").val():0);
    	dataJson.ifUpdateOldData=($(this).find("input[name='ifUpdateOldData'")!=null?$(this).find("input[name='ifUpdateOldData'").val():0);
    	dataJson.ifTrigger=($(this).find("input[name='ifTrigger'")!=null?$(this).find("input[name='ifTrigger'").val():0);
   		return dataJson;
    } else {
    	var str = "";
   		for(var key in json){
    	...
    	str+="&ifCleanInnerData="+($(this).find("input[name='ifCleanInnerData'")!=null?$(this).find("input[name='ifCleanInnerData'").val():0) + "&";
    	str+="&ifOnlyAddInnerData="+($(this).find("input[name='ifOnlyAddInnerData'")!=null?$(this).find("input[name='ifOnlyAddInnerData'").val():0) + "&";
    	str+="&ifUpdateOldData="+($(this).find("input[name='ifUpdateOldData'")!=null?$(this).find("input[name='ifUpdateOldData'").val():0) + "&";
    	str+="&ifTrigger="+($(this).find("input[name='ifTrigger'")!=null?$(this).find("input[name='ifTrigger'").val():0) + "&";
   		return str;
    }
}
 jQuery.extend({   
		      				...
		      				if(fieldRole == 20 || fieldRole == 22) {
		      					...
		      					if (aouInput.parent().hasClass('om-combo')) {
		      						...
		      						if(typeof value == "string") {
			      						...
			      						eval("json.t" + index + "_" + prevName + "[json.t" + index + "_" + prevName + ".length] = '';");
			      					}
		      					} else {
		      							...
		      							if(typeof value == "string") {
			      							...
			      							if(tmpJson != null && tmpJson.length > 0) {
		      					        		for(var lmf = 0; lmf < tmpJson.length; lmf++) {
			      					            	...
			      					            	tmpIdForUpd[tmpIdForUpd.length] = ((tmpJson[lmf].upId==undefined || tmpJson[lmf].upId==0)?"":tmpJson[lmf].upId);
										   	 	}
									    	}
			      						}
		      						} else {
		      					    	if(tmpJson != null && tmpJson.length > 0) {
		      					        	for(var lmf = 0; lmf < tmpJson.length; lmf++) {
			      					        	...
			      					        	for (var key in tmpOneJson) {
			      					         	...
			      					         	tmpIdForUpd[tmpIdForUpd.length] = ((tmpOneJson.upId==undefined || tmpOneJson.upId==0)?"":tmpOneJson.upId);
										   	 }
									    }
		      					    }
                                    ...
                                    for(var lmfI = 0; lmfI < tmpIdForUpd.length; lmfI++) {
			      					...
			      					if(aouInput.length > 0) {
			      						...
			      						tmpChildTable.children("tbody").children().each(function(i, tttOne){
			      							if($(tttOne).find("table").length == 0 || $(tttOne).find("table").attr('id').endWith('_file_upload_fileTable')) {
			      								value ++;
			      							}
			      						});
		      					...
		      					var value = "";
		      					if (aouInput.parent().hasClass("om-combo")) {
		      						value = aouInput.omCombo('value');
		      					} else if (aouInput.attr("type")=="radio"){
		      						value = $("input[name='"+aouInput.attr("name")+"']:checked").val();
		      					} else {
		      						var par = aouInput.closest('.edus_addtdCheckboxBox');
		      						var addValue = function(){
			      						var mySelf = par.find("input[name='"+aouInput.attr("name")+"']:checked");
			      						var mySelfS =par.find("input[name='"+aouInput.attr("name")+"'].checked");
			      						mySelf = mySelf.length ? mySelf : mySelfS;
			      						mySelf.each(function(i){value += (i > 0 ? "," : "") + this.value;});
			      						par[0].myValue = 1;
		      						};
		      						par.length ? (!value || !par[0].myValue) && addValue() :
		      						$("input[name='"+aouInput.attr("name")+"']:checked").each(function (i,one) {
		      							value+=(i>0?",":"")+$(one).val();
		      						});
		      					}
		      					if(typeof value == "string") {
		      					...
		      					if(hidValue != undefined && hidValue != null && hidValue.length > 0 && hidValue>0) {
			      					editorContent = aouInput.attr("hidValueId") + "_wqy_editor_" + editorContent;
		      					} else {
		      						editorContent = editorContent;
		      					}
		      					//var newEditorContent = encodeURIComponent(editorContent);
	      						eval("json.t" + index + "_" + fieldName + "_editor[json.t" + index + "_" + fieldName + "_editor.length] = editorContent;");
	                            ...
	                            eval("json.t" + index + "_" + fieldName + "[json.t" + index + "_" + fieldName + ".length] = value;");
			      				...
			      				//判断是否是富文本 如果是富文本 则处理换行符
			      				if(aouInput[0].tagName == "TEXTAREA") {
			      					value = value.replace(/\n/g, '_@');
			      				}
		      					eval("json.t" + index + "_" + fieldName + "[json.t" + index + "_" + fieldName + ".length] = value;");
		      				}
...
function popupCenter(self,element){//ztx添加弹出层居中定位及窗口高度设置
	var boxsize, myEl;
	if (self || element)if (element){
		myEl = element;
		boxsize = myEl.attr('boxsize');
	}else{
		myEl = self.element;
		boxsize = myEl.attr('boxsize');
	};
	if (boxsize){
		boxsize = boxsize.replace(/-/g,'');
		boxsize = boxsize.split('@');
		var sizeMax = function(size){if (/%$/.test(size)){
			size = parseInt(size);
			size = size > 100 ? 100 : size;
			return size + '%';
		};return size;};
		myEl.removeAttr('boxsize').parent().css({width: sizeMax(boxsize[0]), height: sizeMax(boxsize[1])});
	};
	var options, popup, selfOptionsTime = null, selfOptions = 0, selfOptionsTimeNum = 0;
	if(self){
		options = self.options;popup = self.uiDialog;
	...
	function elemCon(){
		...
		popup = $('.om-dialog.om-widget.om-widget-content.om-corner-all.om-draggable.om-resizable');
	};
	if(element)elemCon();
	if(popup.length >= 0.1){
		popup.each(function(i,b){a = $(b);
			function selfOptionsFn(){
				selfOptions = options.height;
				if(selfOptionsTimeNum>10){clearInterval(selfOptionsTime);return;};selfOptionsTimeNum++;
				...
				thisCon.html().length<=0.1&&a.css({'position': 'absolute','top':$('body').outerHeight()+60});
				if(thisTop.length >= 0.1 && thisCon.length >= 0.1 && (optionsOf?thisBottom.length >= 0.1?true:false:true)){clearInterval(selfOptionsTime);}else{clearInterval(selfOptionsTime); selfOptionsTime = setInterval(selfOptionsFn,100);return;};
				var xzcOf = RegExp('^选择层$').test(thisTop.children('.om-dialog-title').text()),//判断是否是'选择层'。
				HoptiosOF = RegExp('%$').test(selfOptions),
				optiosOFauto = RegExp('auto|undefined').test(selfOptions),
				thisBodyWidth = parseInt($('body').width()),
				thisBodyHeight = parseInt($('body').height()),
				popupHeightH = thisTop.outerHeight() + (optionsOf ? thisBottom.outerHeight() : 0) + (parseInt(thisCon.css('marginTop')) + parseInt(thisCon.css('marginBottom')))||0;
				if(xzcOf){selfOptions = 'auto';};//是‘选择层’就设置为普通显示大小。
				(optiosOFauto||(xzcOf&&thisCon[0].style.width=='auto'||''))&&a.css({'width':'90%'});
				(HoptiosOF||xzcOf||typeof selfOptions=='number')&&a.css({'height':selfOptions,'max-height':thisBodyHeight});
				(HoptiosOF||!optionsOf)&&thisCon.css({'height':99999,'max-height':thisBodyHeight-popupHeightH});//设置自定义弹出层显示宽高不够的默认值。
				var thisBoxTop = (thisBodyHeight-parseInt(a.outerHeight())) * 0.33,//设置普通弹出层距离顶部高度居中偏上。
				thisBoxTop = xzcOf?50:(thisBoxTop<10?10:thisBoxTop);//判断不同类型的弹出层距离顶部的高度距离。
				(!optionsOf||(HoptiosOF&&a[0].style.height=='100%'))&&(thisBoxTop=0);
				var thisBoxHei = thisBodyHeight - thisBoxTop || 0,
				popupLeftA = (thisBodyWidth-parseInt(thisBodyWidth * .9))/2,
				poputLeftB = (thisBodyWidth-a.outerWidth())/2,
				left = parseInt(optiosOFauto?(popupLeftA<=0?0:popupLeftA):(poputLeftB<=0?0:poputLeftB));
				a.css({'top':thisBoxTop,'left':left,'max-height':thisBoxHei,'min-height':150});//设置弹出层显示范围。
				($.browser.msie||$.browser.mozilla)&&thisCon.find('.om-grid.om-widget.om-widget-content').length&&a.css({'height':thisBoxHei});
				var styHeiOf = a[0].style.height=='auto'||'',
				thisConMaxHeight = (styHeiOf?thisBoxHei:a.outerHeight()) - popupHeightH;
				thisCon.css({'max-height':thisConMaxHeight<=0?0:thisConMaxHeight,'min-height':44,'height':(styHeiOf?'':99999)});//设置内容区域的显示范围。
				if(typeof Compatibility == 'function'){var ere = {}; ere.popupCenter = true; ere.xzcOf = xzcOf; Compatibility(ere);};//兼容性修改。
			};
			//过滤后续页面的弹层功能。
			!RegExp('^ztx$').test(a.children().eq(1).attr('afterOperation'))&&selfOptionsFn();
		});
...
function pdivOfZtreeHeight(){
	...
	leftWidgetConPar = $('.om-widget.om-panel.om-resizable.om-borderlayout-region.om-borderlayout-region-west'),
	leftWidgetConParW = 0;
	if(leftWidgetConPar.length >= 0.1){//设置数据字典类型页面左侧的树形列表尺寸。
		if(!leftWidgetConPar.is(':hidden')){
			bodyHeight -= 1;
			leftWidgetConPar.each(function(i,a){
				var leftWidgetConPrev = $(a).children('.om-panel-header.om-borderlayout-region-header'),
				leftWidgetCon = $(a).children('.om-panel-body.om-widget-content'),
				leftWidgetConZtree = leftWidgetCon.children('.ztree'),
				leftWidgetConTreeOper = leftWidgetCon.children('.edus_treeOper');
				leftWidgetConParW = $(a).width();
				$(a).outerHeight(bodyHeight - 1).css('min-width',220);
				leftWidgetConPrev.css({'width':'100%'});
				leftWidgetCon.css('overflow','hidden').css({'width':'100%'})
				.outerHeight(bodyHeight - 1 - leftWidgetCon.prev().outerHeight());
				leftWidgetConZtree.css({'padding': '5px 0px 0px 5px'}).css({'width':''})
				.outerHeight(leftWidgetCon.height() - leftWidgetConTreeOper.outerHeight());
				leftWidgetConTreeOper.css({'top':'auto','left':'auto','right':'auto','bottom':'0px'}).css({'width':'100%'});
				leftWidgetConParW += parseInt($(a).css('marginRight')) + 4;
				if(parseInt($.browser.version) < 8.9){leftWidgetConParW += 2;};
			});
		}else{
			var west = leftWidgetConPar.siblings('.om-borderlayout-proxy.om-borderlayout-proxy-west');
			leftWidgetConParW = west.is(':hidden')?0:(west.width() + 4);
		};
	};
	var widgetContentAll = $('.om-grid.om-widget.om-widget-content');
	if(widgetContentAll.length >= 0.1){//设置表格列表的宽高尺寸。
		widgetContentAll.each(function(bDivI,bDivCon){
			if(!$(bDivCon).hasClass('bDivCon')){
				$(bDivCon).addClass('bDivCon');
				var widgetContent = $(bDivCon),
				boxSizing={'width':'100%','box-sizing':'border-box'},
				widgetContentBox = widgetContent.parent().closest('.om-dialog-content.om-widget-content'),
				//判断当前表格是否在弹层内。
				widgetContentOF=(!widgetContent.parent().closest('.bDiv').length&&!widgetContentBox.length);
				if(RegExp('body|BODY').test(widgetContent.parent()[0].tagName)){widgetContent.css(boxSizing);};
				$('body>.om-widget.om-panel, body>.om-buttonbar, body>.om-dialog-content-box').css(boxSizing);
				widgetContentOF&&(widgetContentBox = $('body'));
				var hDiv = widgetContent.children('.hDiv'),
				bdiv = widgetContent.children('.bDiv'),
				pDiv = widgetContent.children('.pDiv'),
				//获取表格外侧盒子的宽度
				widgetContentBoxW = widgetContentBox.width(),
				widgetContentBoxH = widgetContentBox.height() - 2,
				PrevHeight = {}, BoxHeight = {}, heightAll = 1;
				function thisParentHeight(p,index){
					if(p==null || p==undefined || p.length<=0 || RegExp('body|BODY').test(p[0].tagName) || p.hasClass('om-dialog-content om-widget-content')){return;};
					if(p[0].attributes.id){if(RegExp('box|BOX').test(p[0].attributes.id.value)){return};};
					function thisPrevHeight(pp,indexx){
						if (pp==null || pp==undefined || pp.length<=0 || pp.hasClass('om-widget om-panel om-resizable om-borderlayout-region om-borderlayout-region-west')){return;};
						PrevHeight[pp.outerHeight()] = pp;
						thisPrevHeight(pp.prev(),indexx+1);
					};
					thisPrevHeight(p.prev(),0);
					thisParentHeight(p.parent(),index+1);
					BoxHeight['thisParen_' + index] = p;
				};
				function setBdivW(n){
					//设置框架高度和左侧表格分页的高度。
					var h=widgetContentBoxH - heightAll - hDiv.outerHeight() - pDiv.outerHeight()-n;
					bdiv.outerHeight(h).css(boxSizing);
					bdiv.siblings('.setLeftCon').children('.bDiv').outerHeight(h);
				};
				//设置弹出层内表格的尺寸。
				if(!widgetContentOF){
					var maxHeight=parseInt(widgetContentBox.css('maxHeight'));
					widgetContentBoxH=widgetContentBox.length?parseInt(maxHeight||widgetContentBox.css('height')):widgetContentBoxH;
					thisParentHeight(bdiv.parent(),0);
					for(var ztxI in PrevHeight){heightAll += parseInt(ztxI);};
					setBdivW(6)
					return true;
				};
				//计算主框架的表格尺寸。
				var BoxWidthOf = leftWidgetConParW > 0.1 ? (((widgetContentBoxW - leftWidgetConParW)/widgetContentBoxW*100)-0.2).toFixed(1) : 100;
				widgetContent.outerHeight(widgetContentBoxH - heightAll).css(boxSizing).css({'border':'none','width':'-='+(widgetContent.children('.setLeftCon').outerWidth()||0)});
				thisParentHeight(bdiv.parent(),0);
				function BoxHeightNull(){for(var ztxI in BoxHeight){BoxHeight[ztxI].css({'height':'','width':''});};};
				for(var ztxI in PrevHeight){heightAll += parseInt(ztxI);};
				var widgParent = bdiv.parentsUntil('body');
				widgParent.eq(widgParent.length - 1).css('border','1px solid #ccc');
				//设置表格尺寸。
				if(bdiv.closest('.om-widget.om-panel.om-resizable.om-borderlayout-region.om-borderlayout-region-west').length <= 0.1){
					BoxHeightNull();
					var boxChildren = $('.om-widget.om-panel.om-borderlayout-region.om-borderlayout-region-center');
					if(leftWidgetConParW > 0.1){boxChildren.css({'position':'static','float':'right','border-left':'1px solid #ccc'});};
					if(boxChildren.length >= 0.1){boxChildren.outerHeight(widgetContentBoxH).css({'width':BoxWidthOf + '%','box-sizing':'border-box'})
					.children('#center-panel').outerHeight(widgetContentBoxH).css(boxSizing);};
					setBdivW(0)
				}else{
					setBdivW(0);
					$('#center-panel .edus_search').css({'margin-right':'130px','width':'auto'});
				};
				if(leftWidgetConPar.length <= 0.1){leftWidgetConParW+=2}else{leftWidgetConParW+=3;};
			};
		});
	};
	if(typeof Compatibility == 'function'){var ere = {}; ere.CompatibilityFrame = true; Compatibility(ere);};//兼容性修改。
};
function widgetConHei(){//初始化常用插件及屏幕定位的样式效果。
	if($('head .style_ztx').length <= 0.1){//设置常用插件的盒子。
		var Style = document.createElement('div'),
		head = document.getElementsByTagName('head')[0];
		Style.className = 'style_ztx';
		head.appendChild(Style);
	};
	var style_ztx = $('head .style_ztx')[0];
	if($('.searchNone').length <= 0.1){//设置屏幕弹出层定位的基础框架。
		...
		reportbox = $('body>.reportbox');
		if(gridOf){searchQ = 'padding: 10px; box-sizing: border-box; '};
		if(gridOf || $('.cus_box').length >= 0.1 || reportbox.length >= 0.1){searchJ = 'html body{margin:0; padding:0;' + searchQ + 'overflow: hidden; position:fixed; top:0; left:0; bottom:0; right:0; background:none;}'};
		searchNone.innerHTML = '. <style>.searchNone,.style_ztx{display:none;}' + searchJ + '</style>';
		style_ztx.appendChild(searchNone);
	};
	if($('.om-grid.om-widget.om-widget-content').length <= 0.1 && $('.om-grid.om-widget.om-widget-content').parent().closest('.bDiv').length <= 0.1){}else{$('.om-grid.om-widget.om-widget-content').removeClass('bDivCon');};
	pdivOfZtreeHeight();//设置屏幕列表尺寸大小。
	typeof(Compatibility)=='function'&&Compatibility({initCompatibilityFrame:true});//兼容性修改。
	//添加主页的js模拟滚动条。
	$("#tab0.om-panel-body.om-widget-content.om-panel-noheader.om-state-nobd").omScrollbar({thick:10,toThis:true});
	typeof addJs =='function'&&addJs('titleModel', 'style/js/');
};
function omGridInitAjax(Con){
	if(!Con.self){return;};
	popupCenter();
	pdivOfZtreeHeight();
};
...
if(!window.top.mobile) $(function(){var widgetTint = null, widgetTintIf = 0; clearInterval(widgetTint); widgetTint = setInterval(function(){if(widgetTintIf < 2){widgetConHei();}else{clearInterval(widgetTint);}; widgetTintIf++;},500);});
function outEmojiFn(myText){try{return(typeof(myText)=='string'?decodeURI(myText):myText);}catch(e){return(myText);};};