package com.mbfw.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer {
	...
	public void onMessage(WebSocket conn, String message) {
		...
		if (null != message && message.startsWith("mbfwqq123456789")) {
			this.userjoin(message.replaceFirst("mbfwqq123456789", ""), conn);
		}
		if (null != message && message.startsWith("Leavembfwqq123456789")) {
			this.userLeave(conn);
		}
		if (null != message && message.contains("mbfw886")) {
			String toUser = message.substring(message.indexOf("mbfw886") + 10, message.indexOf("mbfw888"));
			message = message.substring(0, message.indexOf("mbfw886")) + "[私信]  " + message.substring(message.indexOf("mbfw888") + 12, message.length());
